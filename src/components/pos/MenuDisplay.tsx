...
import { Pizza, ChevronRight, Beer, Grape, Droplets } from 'lucide-react';
...
type TabId = 'pizzas' | 'gaseosas' | 'cervezas' | 'bebidas';
...
export function MenuDisplay({ tamanos, toppings, sabores, productos }: MenuDisplayProps) {
  ...
  const tabsVisibles = TABS.filter((tab) => {
    if (tab.id === 'pizzas') return tamanos.length > 0;
    const categoria = TAB_A_CATEGORIA[tab.id];
    return productos.some((p) => p.categoria === categoria);
  });

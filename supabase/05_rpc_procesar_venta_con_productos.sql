-- 05_rpc_procesar_venta_con_productos.sql
-- PARCHE: Extiende procesar_venta para soportar items de tipo "producto"
-- (gaseosas, cervezas, etc.) junto con las pizzas en el mismo carrito.
--
-- REGLA DE NEGOCIO: Si un item del cart_payload tiene tamano_id = NULL,
-- se trata como un producto de precio fijo y se OMITE la deducción de
-- inventario. El item sí queda registrado en cart_payload de la venta
-- para el historial y la reversión.
--
-- El resto de la lógica de pizza permanece idéntico a la versión 03.
...
BEGIN
    ...
    FOR cart_item IN SELECT * FROM jsonb_to_recordset(cart_payload) AS x(
        ...
        -- Se registran en la venta pero no generan ninguna deducción de inventario.
    ...
    -- Si no hubo pizzas en el carrito, las deducciones quedan vacías
    ...
    -- Deducción masiva y atómica del stock de ingredientes (solo pizzas)
